...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:encoder="xalan://java.net.URLEncoder">
	...
	<table align="center" border="0" width="100%" style="border:none;">
		...
		<td style="border:none;">
			<table class="table" border="0" width="100%" style="border:none;">
			...
			<tr style="border:none;">
				<td class="normal_text" align="left" valign="middle" width="100%" style="border:none;">
					<xsl:if test="string-length(../text) &gt; 0">
						<xsl:apply-templates select="text" />
					</xsl:if>
				</td>
				<td class="normal_text" align="left" style="border:none;">
				<xsl:if test="../show_link='true'">
					<xsl:text>&nbsp;</xsl:text>
					...
					<xsl:if test="../show_edition='true'">
						...
						<img class="toolTipImg" border="0" width="16" height="16" src="{$url_prefix}/images/sign.png" alt="Assinar" title="Assinar o documento original" >
															...
															if (isDownloadLinkAvailable('<xsl:value-of select="link_url"/>')){
																var signProObject = parent.document.getElementById('SignPro');
																pathArray = location.href.split( '/' );
																protocol = pathArray[0];
																host = pathArray[2];
																url = location.protocol + '//' + host + '//' + 'iFlow/DocumentService';
																alert(url);
																signProObject.downloadFile(url, document.cookie, document.getElementById('flowid').value, document.getElementById('pid').value, document.getElementById('subpid').value, '<xsl:value-of select="id" />', '<xsl:value-of select="../variable" />');
				...
				<td align="center" valign="middle" style="border:none;"> <!-- add files is enabled but edit is disabled -->
							...
							<div class="multiupload">
								...
								<input id="loadfile" name="loadfile" type="button" value="Carregar" class="button btn btn-default" >
																	<xsl:attribute name="onclick">
																		...
																		host = pathArray[2];
																		url = location.protocol + '//' + host + '//' + 'iFlow/DocumentService';																																		
																		signProObject.escolherDocumentos(url, document.cookie, 
																			document.getElementById('flowid').value, 
																			document.getElementById('pid').value, 
																			document.getElementById('subpid').value, 
